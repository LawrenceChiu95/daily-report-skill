...
| 依赖 | 用途 |
|------|------|
| Node.js / npm | 安装飞书官方 CLI |
| `@larksuite/cli` | 通过 `lark-cli` 读取/创建飞书资源 |
| `jq` | 解析 CLI 返回的 JSON |
| Python 3 | 读取本地 AI 对话记录、GitLab 活动 |
| `curl` | 判断节假日时访问公共 API |
...
setup 会自动申请常用飞书权限域：`calendar,docs,drive,contact,vc,im`，并检查消息搜索相关 scope：`search:message`、`im:message.group_msg:get_as_user`；如果完整模式开启私聊采集，还会检查 `im:message.p2p_msg:get_as_user`。

配置文件权限会被限制为 `600`。如果启用 GitLab 采集，Token 会写入该本地配置文件，请只在自己的电脑上启用。
